...
/**
  ...
  */
class ClientCommandHandler(phoneNumber: String, userName: String) extends PersistentActor
  with ActorLogging with TwitterCredentials {

  implicit val ec = context.dispatcher

  override def postStop(): Unit = subscriptionScheduler.foreach(_.cancel())
  ...
  override def receiveCommand = {
    case SubscribeMentions(_) =>
      if (subscriptionScheduler.isDefined) sender() ! InvalidCommand("Already subscribed to mentions")
      else persist(MentionsSubscribed())(handleEvent)
    ...
    case AcknowledgeMention(id) => persist(MentionAcknowledged(id))(handleEvent)
  ...
  def fetchMentions(consumerKey: ConsumerKey, requestToken: RequestToken, user: String, time: DateTime):
  Future[Seq[(String, DateTime, String, String)]] = {
